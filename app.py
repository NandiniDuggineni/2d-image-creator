...
def create_scene_image(description, width=400, height=300):
    # Simple image with background color
...
desc = st.text_area("Enter scene description or keywords (e.g. 'sun, tree, grass'):")
...
if st.button("Generate Image"):
    if not desc.strip():
        ...
        save_path = f"static/{desc.strip().replace(' ', '_')[:20]}.jpg"
        img.save(save_path)
